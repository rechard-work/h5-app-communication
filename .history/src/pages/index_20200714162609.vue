<template>
  <div class='pages'>
    ...
    <van-swipe ref="myswiper" class="my-swipe" :loop='false' :initial-swipe='current' @change="onChange">
      ...
      <template v-for="(item, index) in slideArr">
        <van-swipe-item class="flex" :key="index" v-if="index===0">
          <div class="flex wrapper" ref="wrapper">
            <div class="content">
              <h1>{{ msg }}</h1>
              <h2>Essential Links</h2>
        ...
        <van-swipe-item v-if="index>=1" :key="index">{{index+1}}</van-swipe-item>
      </template>
      <template #indicator>
